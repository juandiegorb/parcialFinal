/*
*Programa: Suspuesto sistema de biblioteca
*Creadores: Daniel Steven Agudelo, Sergio Daniel Arias, Samuel Fernandez Fernandez, Juan Diego Rios 
*Fecha de ultima modificacion: 28/11/2018
*
*/
// librerias
//string.h
...
#define usuarioadmin "h1"
// Constante contraseña del administrador
#define contrasenaadmin "h"

//Prototipo de la funcion contador
void contador(struct libreria libros[1000], int *p_contadorlibros, int conta_dupli[100000], int diaActual, int mesActual, int annoActual);

//structura del auxiliar para ordenamiento
struct auxiliar{
	//Variable auxCodigo
	int auxCodigo;
	//Variable auxNombre de 50 posiciones
	char auxNombre[50];
	//Variable auxAutorLibro de 50 posiciones
	char auxAutorLibro[50];
	//Variable auxGeneroLibro de 50 posiciones
	char auxGeneroLibro[50];
	//Variable auxEditorialLibro de 50 posicoines
	char auxEditorialLibro[50];
	// aux fecha publicacion
	//Variable auxDiaPublicacion
	int auxDiaPublicacion;
	//Variable auxMesPublicacion
	int auxMesPublicacion;
	//Variable auxAnnoPublicacion
	int auxAnnoPublicacion;
	// aux fecha Ingreso
	//Variable auxDiaIngreso
	int auxDiaIngreso;
	//Variable auxMesIngreso
	int auxMesIngreso;
	//Variable auxAnnoIngreso 
	int auxAnnoIngreso;
	// aux fecha prestamo
	//Variable auxDiaPrestamo
	int auxDiaPrestamo;
	//Variable auxMesPrestamo
	int auxMesPrestamo;
	//Variable auxAnnoPrestamo
	int auxAnnoPrestamo;
	// aux fecha devolucion
	//Variable auxDiaDevolucion
	int auxDiaDevolucion;
	//Variable auxMesDevolucion
	int auxMesDevolucion;
	//Variable auxAnnoDevolucion
	int auxAnnoDevolucion;
}aux; // Nombre para llamar la struct

// struct para prestamos de los libros
struct prestamos{
	//Variable Identificacion de 20 posiciones
	char Identificacion[20];
	//Variable Nombrepersona de 30 posiciones
	char Nombrepersona[30];
	//Variable Celular de 10 posiciones 
	char Celular[10];
	//Variable de CantidadMax de libros, Valor de 2
	int CantidadMax = 2;
	//Variable de CantidadActual, Valor de 0
	int  CantidadActual = 0;
	// fecha prestamo de libro
	// Variable del diaPrestamo
	int diaPrestamo;
	//Variable del mesPrestamo
	int mesPrestamo;
	//Variable del annoPrestamo
	int annoPrestamo;
	
}Registro[1000]; //Nombre para llamar el struct
// struct que contiene la informacion de los libros
struct libreria{
	// variable ID
	int ID;
	// variable codigoLibro
	int codigoLibro;
	// variable nombreLibro
	char nombreLibro[50];
	// variable autorLibro
	char autorLibro[50];
	// variable generoLibro
	char generoLibro[50];
	// variable editorialLibro
	char editorialLibro[50];
	
	// fecha publicacion del libro
	// variable diaPublicacion
	int diaPublicacion;
	// variable mesPublicacion
	int mesPublicacion;
	// variable annoPublicacion
	int annoPublicacion;
	...
	// variable mesIngreso
	int mesIngreso;
	// variable annoIngreso
	int annoIngreso;
	
	// fecha devolucion del libro
	// variable diaDevolucion
	int diaDevolucion;
	// variable mesDevolucion 
	int mesDevolucion;
	// variable annoDevolucion
	int annoDevolucion;
}libros[1000];//nobre para llamar la struct

// funcion para contar la cantidad de libros en el inventario 
void contador_inventario(struct libreria libros[1000], int *p_contadorlibros, int conta_dupli[100000], int diaActual, int mesActual, int annoActual){
	...
	do{
		for(int j=i+1; j<*p_contadorlibros; j++){
			if(libros[i].codigoLibro == libros[j].codigoLibro){
				conta_dupli[libros[i].codigoLibro] = conta_dupli[libros[i].codigoLibro] + 1;	
			}			
		}		
				...
				printf("Fecha de ingreso del libro a la Base de Datos(DD/MM/YYYY): %d/%d/%d\n",
					   ...
					   mesActual, 
					   annoActual);					
				printf("Cantidad de libros %d\n", conta_dupli[libros[i].codigoLibro]);
		i = i + conta_dupli[libros[i].codigoLibro];
	}while(i < *p_contadorlibros);	
}

//Contador para contar, disminuir los valores en caso de que se modifique 
void Contador_pedir(struct libreria libros[1000], int *p_contadorlibros, int conta_dupli[100000],int *p_contaLibroD, int *p_diaActual, int *p_mesActual, int *p_annoActual){
	int bandera = 1;
	if(bandera == 1){
		for(int i=0; i< 100000; i++){
			conta_dupli[i] = 1;
	...
	int  codigoDeseado;
	int cant_pres=0, cant_deseada, cant_exist=0;
	printf("ingrese el codigo del libro que desea prestar (si no esta seguro de su existencia"
		   "en el inventario, por favor ingrese a la opcion 2 para ver el inventario y conocer el codigo ");
	
			scanf("%d", &codigoDeseado);
			
			for(int i = 0; i<*p_contadorlibros; i++){
				if(codigoDeseado == libros[i].codigoLibro){
					cant_pres = (cant_pres + 1);
				}
			}
			printf("la cantidad de unidades del libro solicitado son: %d\n\n", cant_pres);
			printf("teniendo en cuenta la cantidad existente, cuantos desea prestar");
			scanf("%d", &cant_deseada);
			
			cant_exist = (cant_pres - cant_deseada);
			
				for(int i=0; i<*p_contadorlibros; i++){
				if(codigoDeseado == libros[i].codigoLibro){
					conta_dupli[codigoDeseado]=cant_exist;
				}
			}
				printf("Cantidad restante en el inventario del libro %d", conta_dupli[codigoDeseado]);
	getch();
	system("cls");
}

//Contador inicial para los 50 libros existentes
void contador(struct libreria libros[1000], int *p_contadorlibros, int conta_dupli[100000], int *p_diaActual, int *p_mesActual, int *p_annoActual){
	for(int i=0; i< 100000; i++){
		conta_dupli[i] = 1;
	}		
	int i = 0;
...
void ordenamiento(struct libreria libros[1000], int *p_contadorlibros){
	for(int i=0; i< *p_contadorlibros; i++){
		for(int j=i+1; j<*p_contadorlibros; j++){
			if(libros[i].codigoLibro == libros[j].codigoLibro){
				// aux datos libros
				aux.auxCodigo = libros[i+1].codigoLibro;
				strcpy(aux.auxNombre, libros[i+1].nombreLibro);
				strcpy(aux.auxAutorLibro, libros[i+1].autorLibro);
				strcpy(aux.auxGeneroLibro, libros[i+1].generoLibro);
				strcpy(aux.auxEditorialLibro, libros[i+1].editorialLibro);
				// aux fecha publicacion
				aux.auxDiaPublicacion = libros[i+1].diaPublicacion;
				aux.auxMesPublicacion = libros[i+1].mesPublicacion;
				aux.auxAnnoPublicacion = libros[i+1].annoPublicacion;
				// aux fecha Ingreso
				aux.auxDiaIngreso = libros[i+1].diaIngreso;
				aux.auxMesIngreso = libros[i+1].mesIngreso;
				aux.auxAnnoIngreso = libros[i+1].annoIngreso;
				// aux fecha devolucion
				aux.auxDiaDevolucion = libros[i+1].diaDevolucion;
				aux.auxMesDevolucion = libros[i+1].mesDevolucion;
				aux.auxAnnoDevolucion = libros[i+1].annoDevolucion;													
				
				// libro igual al comparado remplazo el libro siguiente al del libro a comparar(i+1) con el de la posicion encontrado igual(j)
				libros[i+1].codigoLibro = libros[j].codigoLibro;
				strcpy(libros[i+1].nombreLibro, libros[j].nombreLibro);
				strcpy(libros[i+1].autorLibro, libros[j].autorLibro);
				strcpy(libros[i+1].generoLibro, libros[j].generoLibro);
				strcpy(libros[i+1].editorialLibro, libros[j].editorialLibro);
				
				libros[i+1].diaPublicacion = libros[j].diaPublicacion;
				libros[i+1].mesPublicacion = libros[j].mesPublicacion;
				libros[i+1].annoPublicacion = libros[j].annoPublicacion;
				
				libros[i+1].diaIngreso = libros[j].diaIngreso;
				libros[i+1].mesIngreso = libros[j].mesIngreso;
				libros[i+1].annoIngreso = libros[j].annoIngreso;
				
				libros[i+1].diaDevolucion = libros[j].diaDevolucion;
				libros[i+1].mesDevolucion = libros[j].mesDevolucion;
				libros[i+1].annoDevolucion = libros[j].annoDevolucion;
				// remplazo la posicion j con el de i+1
				
				libros[j].codigoLibro = aux.auxCodigo;
				strcpy(libros[j].nombreLibro, aux.auxNombre);
				strcpy(libros[j].autorLibro, aux.auxAutorLibro);
				strcpy(libros[j].generoLibro, aux.auxGeneroLibro);
				strcpy(libros[j].editorialLibro, aux.auxEditorialLibro);
				
				libros[j].diaPublicacion = aux.auxDiaPublicacion;
				libros[j].mesPublicacion = aux.auxMesPublicacion;
				libros[j].annoPublicacion = aux.auxAnnoPublicacion;
				
				libros[j].diaIngreso = aux.auxDiaIngreso;
				libros[j].mesIngreso = aux.auxMesIngreso;
				libros[j].annoIngreso = aux.auxAnnoIngreso;
				
				libros[j].diaDevolucion = aux.auxDiaDevolucion;
				libros[j].mesDevolucion = aux.auxMesDevolucion;
				libros[j].annoDevolucion = aux.auxAnnoDevolucion;												
			}
		}
	}
	system("cls");
...
// funcion para agregar los 50 libros que deben estar en la biblioteca
void libros_antiguos(struct libreria libros[1000], int *p_contadorlibros, int conta_dupli[100000], int *p_diaActual, int *p_mesActual, int *p_annoActual){
	
	for(int i=0; i<50; i++){
		libros[i].ID = i+1;
		libros[i].diaIngreso = 10;
		libros[i].mesIngreso = 5;
		libros[i].annoIngreso = 2010;
	}
	///LIBRO 1
	...
	ordenamiento(libros, p_contadorlibros);
	contador(libros, p_contadorlibros, conta_dupli, p_diaActual, p_mesActual, p_annoActual);	
}

//Funcion anexar libros
void Anexar_libros(int *p_contadorlibros, int &id, int *p_diaActual,int *p_mesActual, int *p_annoActual,int conta_dupli[100000]){
...
void menus(struct libreria libros[1000], struct auxiliar aux){
	//Variables
	int seleccionLogin, seleccionMenu, seleccionConsulta, contadorLibros, diaActual, mesActual, annoActual, id = 51, contaViejos, contaNuevos, conta_dupli[100000];
	int contaLibroD[100000], consultaNum;
	char nomAdmin[30], contraAdmin[30], consultas[50];
	// Punteros
	int *p_selLogin, *p_contadorlibros, *p_diaActual, *p_mesActual, *p_annoActual, *p_contaViejos, *p_contaNuevos, *p_contaLibroD;
	...
	p_contaLibroD = contaLibroD;
	
	*p_contaViejos = 0;
	*p_contaNuevos = 0;
	*p_contadorlibros = 50;
	
	libros_antiguos(libros, p_contadorlibros,conta_dupli, p_diaActual, p_mesActual, p_annoActual);
							...
							do{
								...
								switch(seleccionMenu){
									...
									case 1:{				
										...
										Anexar_libros(p_contadorlibros, id, p_diaActual, p_mesActual,p_annoActual, conta_dupli);
										break;
									}
									case 2:{
										for(int i = 0; i < *p_contadorlibros; i++){
										...
										*p_contaViejos = 0;
										*p_contaNuevos = 0;																		
										contador_inventario(libros, p_contadorlibros, conta_dupli, diaActual, mesActual, annoActual);
										getch();
										system("cls");
										break;
									}
									case 3:{
										Contador_pedir(libros, p_contadorlibros, conta_dupli,p_contaLibroD, p_diaActual, p_mesActual, p_annoActual);
										break;
									}
									case 4:{
											do{
												...
												printf("Seleccione una opcion de consulta"
													   ...
													   "\n2. Consulta por nombre del libro"
													   "\n3. Consulta por nombre de autor"
												...
												switch(seleccionConsulta){
													case 1:{
														...
														do{
															int c = 0;
															system("cls");
															printf("Generos(Nota: al ingresar el genero debe ser con Mayuscula inicial)\n"
																   "Romance\n"
																   "Fantasia\n"
																   "Ciencia Ficcion\n"
																   "Militar\n"
																   "Novela\n"
																   "Suspenso\n");															
															printf("Ingrese el genero del libro a buscar: ");
															fflush(stdin);
															gets(consultas);
															for(int j=0; j<*p_contadorlibros; j++){
																int bus1 = strcmp(consultas, libros[j].generoLibro);
																if(bus1 == 0){
																	printf("Codigo libro: %d\n", libros[j].codigoLibro);
																	printf("Nombre del libro: %s\n",libros[j].nombreLibro);
																	printf("Autor del libro: %s\n",libros[j].autorLibro);
																	printf("Genero del libro: %s\n",libros[j].generoLibro);
																	printf("Editorial del libro: %s\n",libros[j].editorialLibro);
																	printf("Fecha de publicacion del libro(DD/MM/YYYY): %d/%d/%d\n", 
																		   libros[j].diaPublicacion, 
																		   libros[j].mesPublicacion, 
																		   libros[j].annoPublicacion);
																	c++;
																}																																						
															}
															if(c == 0){
																printf("No hay libros con ese genero o esta mal digitado\n");
															}
															printf("Desea realizar otra busqueda?(1. Volver a buscar 0. Salir): ");
															scanf("%d", &seleccionbusqueda);
														}while(seleccionbusqueda!=0);
													...
													case 2:{
														...
														do{
															...
															gets(consultas);
															for(int j=0; j<*p_contadorlibros; j++){
																int bus1 = strcmp(consultas, libros[j].nombreLibro);
																if(bus1 == 0){
																	printf("Codigo libro: %d\n", libros[j].codigoLibro);
																	printf("Nombre del libro: %s\n",libros[j].nombreLibro);
																	printf("Autor del libro: %s\n",libros[j].autorLibro);
																...
																if(c == 0){
																	printf("No hay libros con ese nombre o esta mal digitado\n");
																}
																printf("Desea realizar otra busqueda?(1. Volver a buscar 0. Salir): ");
																scanf("%d", &seleccionbusqueda);
															}while(seleccionbusqueda!=0);													
													...
													case 3:{
															...
															do{
																...
																gets(consultas);
																for(int j=0; j<*p_contadorlibros; j++){
																	int bus1 = strcmp(consultas, libros[j].autorLibro);
																	if(bus1 == 0){
																		printf("Codigo libro: %d\n", libros[j].codigoLibro);
																		printf("Nombre del libro: %s\n",libros[j].nombreLibro);
																		printf("Autor del libro: %s\n",libros[j].autorLibro);
																...
																if(c == 0){
																	printf("No hay libros con ese autor o esta mal digitado\n");
																}
																printf("Desea realizar otra busqueda?(1. Volver a buscar 0. Salir): ");
																scanf("%d", &seleccionbusqueda);
															}while(seleccionbusqueda!=0);													
													...
													case 4:{
																...
																do{
																	int c = 0;
																	system("cls");
																	printf("Ingrese la editorial del libro buscar: ");
																	fflush(stdin);
																	gets(consultas);
																	for(int j=0; j<*p_contadorlibros; j++){
																		int bus1 = strcmp(consultas, libros[j].editorialLibro);
																		if(bus1 == 0){
																			printf("Codigo libro: %d\n", libros[j].codigoLibro);
																			printf("Nombre del libro: %s\n",libros[j].nombreLibro);
																			printf("Autor del libro: %s\n",libros[j].autorLibro);
																	...
																	if(c == 0){
																		printf("No hay libros con esa editorial\n");
																	}
																	printf("Desea realizar otra busqueda?(1. Volver a buscar 0. Salir): ");
																	scanf("%d", &seleccionbusqueda);
																}while(seleccionbusqueda!=0);													
													...
													case 5:{
																	...
																	do{
																		int c = 0;
																		system("cls");
																		printf("Ingrese el anno de publicacion del libro a buscar: ");
																		scanf("%d,", &consultaNum);
																		for(int j=0; j<*p_contadorlibros; j++){
																		
																			if(consultaNum == libros[j].annoPublicacion){
																				printf("Codigo libro: %d\n", libros[j].codigoLibro);
																				printf("Nombre del libro: %s\n",libros[j].nombreLibro);
																				printf("Autor del libro: %s\n",libros[j].autorLibro);
																				printf("Genero del libro: %s\n",libros[j].generoLibro);
																		...
																		if(c == 0){
																			printf("No hay libros con ese anno de publicacion\n");
																		}
																		printf("Desea realizar otra busqueda?(1. Volver a buscar 0. Salir): ");
																		scanf("%d", &seleccionbusqueda);
																	}while(seleccionbusqueda!=0);													
